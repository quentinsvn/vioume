...
$shop = $bdd->query('SELECT * FROM products');
...
<body>
    ...
    <main>
        <div class="container mt-4">
            <div class="row">
                <div class="col-md-12">
                    <?php while($s = $shop->fetch()) { 
                        $phpdate = strtotime($s['date_product']);
                        $s['date_product'] = date( 'd/m/Y', $phpdate );
                    ?>
                    <div class="card mb-3">
                        <div class="row no-gutters">
                            <div class="col-md-4">
                            <img src="../assets/uploads/products/<?php echo $s['miniature'] ?>" class="card-img">
                            </div>
                            <div class="col-md-8">
                                <div class="card-body">
                                    <h5 class="card-title"><?php echo $s['name'] ?></h5>
                                    <p class="card-text"><?php echo $s['description'] ?>
                                    </p>
                                    <h5><?php echo $s['price'] ?> Vioucoins</h5>
                                    <a href="details/index.php?id=<?php echo $s['id'] ?>">Commander →</a>
                                </div>
                            </div>
                        </div>
                    </div>
